...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === "showInterpretations") {
      // 在当前页面显示解释列表
      showInterpretations(request.word);
      console.log(request.word)
    }
  });
  
  // 显示解释列表
  function showInterpretations(word) {
    // 创建一个固定在页面上的容器
    ...
    // word = '123123'
    // 添加解释内容
    var interpretations = getInterpretations(word); // 获取解释内容的函数，需要自行实现
  ...
  // 获取解释内容的示例函数
  function getInterpretations(word) {
